...
(put 'basic-faces 'custom-loads '(button faces frame fringe isearch mouse replace shadowfile tooltip))
...
(put 'convenience 'custom-loads '(abbrev autoinsert autorevert bs "buff-menu" completion cua-base cus-edit dabbrev display-line-numbers emacs-lock ffap filecache files filesets follow hippie-exp hl-line ibuffer ido imenu kmacro linum org-protocol register repeat reveal ruler-mode simple speedbar vcursor whitespace windmove))
...
(put 'display 'custom-loads '("bindings" "characters" display-line-numbers face-remap faces menu-bar simple))
...
(put 'minibuffer 'custom-loads '(icomplete "map-ynp" minibuf-eldef minibuffer rfn-eshadow savehist simple))
...
(put 'mouse 'custom-loads '(artist avoid goto-addr mouse msb mwheel strokes tooltip))
(put 'mpc 'custom-loads '(mpc))
(put 'mpuz 'custom-loads '(mpuz))
(put 'msb 'custom-loads '(msb))
(put 'mspools 'custom-loads '(mspools))
(put 'mule 'custom-loads '(descr-text kkc latexenc misc-lang mule "mule-cmds" select))
...
(defvar custom-versions-load-alist '(("19.29" time-stamp) ("20" ebnf2ps ps-print) ("20.1" time-stamp) ("20.3" avoid which-func xscheme ffap add-log compile rmail uce hexl vcursor browse-url dabbrev cus-edit simple paren msb find-func mouse replace sendmail debug diary-lib) ("20.3.3" dos-vars) ("20.4" sh-script hilit-chg files compile) ("20.8" sql) ("21.1" dabbrev rmailedit latin1-disp ezimage smtpmail frame gnus-start hideshow strokes sb-image vc-sccs tooltip net-utils browse-url debug fortran font-lock cperl-mode isearch mouse cal-hebrew sgml-mode server gnus-group vc-cvs "window" vc rmail gnus-sum sendmail files gnus-agent cus-edit paren find-func vc-rcs nnmail add-log simple ps-print etags flyspell gnus-art ange-ftp wid-edit message faces) ("21.3" gnus-agent ange-ftp sql) ("22" ps-print ebnf2ps) ("22.1" allout imenu binhex tcl imap sh-script gnus-srvr mail-source facemenu minibuffer spam sgml-mode generic-x gnus-int dabbrev mm-view locate mouse fortran frame glasses sql gnus-win view vc-svn mm-uu descr-text ielm ffap vc gnus-score diary-lib rmail gnus-group mule mml2015 ispell make-mode files gnus-msg shadowfile x-dnd bookmark uudecode mm-util "buff-menu" cal-tex compare-w mml find-func tls autorevert mml-sec add-log info hl-line dired custom org-faces smime fringe flow-fill mail-extr ange-ftp desktop starttls diff-mode mm-url pop3 time cal-dst sendmail gnus-fun comint mm-decode grep gnus-agent latin1-disp vc-cvs nnmail gnus-delay warnings gnus-cite replace gnus-art server deuglify gdb-mi cus-edit rfn-eshadow simple hi-lock faces tex-mode dnd gnus-sum compile message keypad gnus gnus-util calendar gnus-start) ("22.2" sql gdb-mi tls vc-mtn gnus-sum css-mode bibtex-style dired desktop vc-hg find-dired mail-source) ("22.3" message) ("23.1" descr-text fortune gdb-mi newcomment smiley server imap nnmail gnus-draft "mule-cmds" gnus-util gnus-dired copyright replace faces cus-edit mm-decode mml checkdoc dired-aux smime rmailmm gnus-bookmark view vc-git misearch pcmpl-unix select info f90 compile gnus-msg filesets rmail gnus gnus-group informat allout help spam-report vc-cvs vc-mtn gnus-score bibtex sendmail simple mm-uu tls hideif face-remap fortran files ffap holidays message gnus-art isearch gnus-sum minibuffer time browse-url tex-mode nnmairix vc-hg pop3 woman calendar "startup" "window") ("23.2" files diary-lib tramp-gvfs "window" rmailmm gnus-sum help-fns ansi-color bytecomp gnus-group mm-encode rmail calendar simple lunar dired elint message comint auth-source minibuffer desktop doc-view gdb-mi) ("23.3" desktop rmailsum calendar rmail diary-lib) ("23.4" ede) ("24.1" facemenu em-ls esh-cmd package-x esh-io ob-exp org-src cc-vars em-smart esh-proc em-prompt ob-R ob-picolisp esh-arg ob-plantuml org-crypt vc-cvs em-term log-edit em-hist erc-button copyright f90 simple erc-backend esh-mode minibuffer em-cmpl woman etags dirtrack em-pred gnus-msg ob-lisp org-attach em-unix dired-x "characters" cc-fonts em-banner hl-line man ibuffer cus-theme esh-ext sieve-manage em-script gnus-registry gnus-srvr ido auth-source battery emacs-lock gssapi netrc gnus-art ange-ftp find-dired doc-view smtpmail font-lock em-dirs mm-decode flyspell jka-cmpr-hook mml vc-svn js org-mobile select calendar vc-bzr gnus-group vc-rcs em-alias faces erc-join gnus-picon elec-pair glasses message em-rebind esh-var mml2015 shr vc gnus-html ob-tangle diary-lib calc org-habit saveplace compile org-capture url-cache image-mode mouse ses esh-util allout gnus-sum files org-ctags sendmail pcmpl-unix tramp org-entities org-clock info org-inlinetask arc-mode url-queue nnir ob-ditaa ob-core sql appt vc-sccs server ox-icalendar rcirc cl-indent gnus-async rmail smiley package cc-guess gnus-util org-archive browse-url custom org-table em-glob gnus "window" cus-edit allout-widgets ob-js gnus-gravatar org-faces bibtex mule vc-git ox-odt gnus-int org-agenda pop3 org-list verilog-mode prolog org gnus-start feedmail mm-view org-bibtex rst) ("24.2" which-func) ("24.3" image ediff-wind perl-mode em-term proced ffap org-id rcirc "bindings" message erc-backend ido calendar server tramp ox-odt help avoid "mule-cmds" which-func wdired bug-reference calc newcomment isearch pcmpl-gnu simple pcmpl-rpm icalendar gnus-picon ob-C ob-groovy sh-script org-capture cal-html "buff-menu" vhdl-mode hexl diff-mode ob-java minibuf-eldef gdb-mi flymake debug tramp-smb files cus-edit erc-match org-agenda gnus-msg sql smerge-mode allout apropos reftex-vars verilog-mode semantic/complete nnmail compile replace org emacs-lock org-datetree man) ("24.4" ob-core bibtex em-term mml2015 uniquify tramp-cache gdb-mi message help epa recentf gnus-start f90 tramp-adb strokes "indent" calc timeclock man info register autorevert imenu idlwave sql org-gnus term jka-cmpr-hook ede/linux tramp-smb cookie1 org-protocol menu-bar flymake-proc dired isearch ansi-color tramp-sh mm-util cfengine saveplace vhdl-mode org-archive sh-script rlogin hi-lock org-clock nnimap sieve-manage gnus-art ox-beamer verilog-mode js rmail flymake org-src apropos info-xref type-break ob-ruby m4-mode frame bookmark flyspell log-edit ox-texinfo unrmail replace org-attach smie gnus mml-smime ob-ocaml "window" gnutls reftex-vars org-mobile pop3 shr "bindings" files pcmpl-x desktop ob-ditaa quickurl python eww simple org gnus-sum compare-w ob-lilypond ox-man ob-python org-faces icomplete vc-svn ox-html remember calendar ox tramp octave faces doc-view org-agenda ox-odt ox-latex elec-pair package find-file ido ruby-mode ox-ascii) ("24.5" flyspell vc-hg erc-stamp erc mml2015) ("25.1" checkdoc js minibuffer rcirc sh-script info simple vc-cvs url-tramp arc-mode em-term sgml-mode message vhdl-mode sql mml-sec select ldap filecache vc-svn gnus-sum newst-reader vc gud autoinsert prog-mode gnus-art rmailmm newst-treeview faces hideif find-func mm-decode cl-indent vc-hg eudc-vars gnus-fun gnus-registry isearch calendar "paragraphs" epg-config ls-lisp erc nsm electric gdb-mi erc-desktop-notifications url-handlers python check-declare xt-mouse whitespace compare-w "window" tildify vc-mtn net-utils epa vc-bzr frame rect eww paren tramp-adb diff-mode shr verilog-mode tex-mode vc-hooks icalendar files replace browse-url vc-git package vc-annotate) ("25.2" tramp-sh ffap) ("26.1" org-timer man quickurl ob-scheme enriched org-faces css-mode package ob-exp ob-vala "bindings" flyspell mwheel debug epa sql battery term mhtml-mode cpp dns-mode checkdoc browse-url ibuffer org-colview ox-md electric grep perl-mode icomplete executable gdb-mi tex-mode ibuf-ext dired image org-bibtex dunnet ox-odt bibtex sieve-manage ox-texinfo erc tramp-sh tooltip wdired ox-beamer tls org-attach tramp message org-duration org-src misc-lang python ediff-init mailcap add-log comint gnus-mh replace ob-lua ob-clojure ob-core ange-ftp shr org-clock ruby-mode org-table simple faces ox-ascii display-line-numbers image-dired org-bbdb ob-lisp ox-html net-utils org mouse "fill" gnus-art vc-git cc-vars gnus-cloud files ox flymake url-vars dired-x tramp-gvfs ob-J ox-latex edebug vc-hg "window" ox-publish js org-agenda) ("26.2" imenu "map-ynp" mail-source image calc shadowfile) ("26.3" bytecomp tramp-sh help-fns))
